...
def TemplFeaturizeFixbb(seq, conf_1d=None):
    """  
    用于固定 BB 示例的 1D 特征化模板:
    Parameters:
        seq (torch.tensor, required): Integer sequence 
        conf_1d (torch.tensor, optional): 预计算置信度张量
    """
    # L = seq.shape[-1]
...
def MSAFeaturize_fixbb(msa, params):
    '''
    输入：完整的 MSA 信息。RFDiffusion的MSA实际上就是含有掩码的单个序列，形状为[1, len]
    输出：单一序列，其中一定比例的氨基酸发生了突变（但不残留 “屏蔽 ”的氨基酸）
    
    这是在autofold2的基础上修改而来的，以去除单一序列的突变
    '''
    # N, L = msa.shape
    # 原始 MSA 剖面
    ...
    for i_cycle in range(params['MAXCYCLE']):
        ...
        msa_onehot = torch.nn.functional.one_hot(msa[:1], num_classes=22)  # 对MSA的每个字符进行one-hot编码
        # 添加额外的两个 indel 平面（解释：https://lxblog.com/qianwen/share?shareId=ab58604c-e892-4f6c-b8ef-d48342d451e2），并将其设置为零。每一个平面包含插入和删除
        msa_fakeprofile_onehot = torch.nn.functional.one_hot(msa[:1], num_classes=26)
        # 将MSA的one-hot编码和包含两个 indel 平面的one-hot编码合并
        ...
        # 伪造 msa_clust 和 mask_pos
        msa_clust = msa[:1]
        mask_pos = torch.full_like(msa_clust, 1).bool()
        # 形成批次的MSA相关信息
        b_seq.append(msa[0].clone())
        b_msa_seed.append(msa_full_onehot[:1].clone())  # 屏蔽的单序列单次（注：没有屏蔽，因此只是单序列单次）
        b_msa_extra.append(msa_extra_onehot[:1].clone())  # 屏蔽的单序列单次（注：没有屏蔽，因此只是单序列单次）
        b_msa_clust.append(msa_clust[:1].clone())  # 未掩盖的原始单序列
        b_mask_pos.append(mask_pos[:1].clone())  # 单一序列中的屏蔽位置（全为零）
