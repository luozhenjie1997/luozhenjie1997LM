...
if __name__ == '__main__':
    config, config_name = load_config('configs/base_pretrain_onlyCDR.yaml')
    ...
    if os.path.exists('./save_model/onlyCDR_checkpoint.ckpt'):
        ...
        val_log_step = 1
        best_loss = 999.
        log_name = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + '(onlyCDR)'
    ...
    """生成HCDR3任务索引"""
    hcdr3_flags = torch.zeros(config.train.max_iters, device=device)
    num_ones = int(config.train.max_iters * 0.2)
    ones_indices = torch.randperm(config.train.max_iters)[:num_ones]  # 随机采样索引
    hcdr3_flags[ones_indices] = 1
    ...
    while True:
        ...
        if hcdr3_flags[it] == 1:
            ...
            cdr_flag = batch['cdr_flag'].bool()
        batch['generate_mask'] = torch.logical_and(cdr_flag, batch['generate_mask'])
        ...
        if random.random() > 0.5:
            with torch.no_grad():
                output = model(batch, only_cdr=True)
                # 获取自条件
                batch['pred_trans_1'] = output['pred_trans_1']
                batch['pred_bb_atom'] = output['pred_bb_atom']
        output = model(batch, only_cdr=True, use_checkpoint=use_checkpoint)
            ...
            if it % config.train.val_freq == 0 and batch_count == 0:
                ...
                with torch.no_grad():
                    for val_batch in val_loader:
                        ...
                        val_batch['generate_mask'] = torch.logical_and(val_batch['cdr_flag'].bool(), val_batch['generate_mask'])
                ...
                if val_batch_loss < best_loss:
                    ...
                    torch.save(model.state_dict(), './save_model/onlyCDR_model.pt')
                ...
                torch.save(checkpoint_dict, './save_model/onlyCDR_checkpoint.ckpt')
                # eval之后，恢复原来模型的参数
                # ema.train()
                model.train()
